...
// Generasi 150 Responden Seimbang (75 Laki-laki & 75 Perempuan) Siap Uji Independent T-Test di SPSS
export function generate150BalancedTTestRespondents(): RespondentData[] {
  const today = new Date().toISOString().split('T')[0];
  const results: RespondentData[] = [];
  const maleNames = ['Ahmad', 'Budi', 'Chandra', 'Deni', 'Eko', 'Fajar', 'Giri', 'Hadi', 'Irfan', 'Joko', 'Kiki', 'Lukman', 'Mahendra', 'Noval', 'Oki', 'Pratama', 'Qori', 'Rian', 'Suryo', 'Taufik', 'Utomo', 'Vino', 'Wahyu', 'Xander', 'Yoga'];
  const femaleNames = ['Anisa', 'Bunga', 'Citra', 'Dewi', 'Eka', 'Fitri', 'Gita', 'Hani', 'Indah', 'Jelita', 'Kartika', 'Laras', 'Maya', 'Nabila', 'Oliva', 'Putri', 'Qonita', 'Rina', 'Siti', 'Tania', 'Ulfa', 'Vina', 'Wulan', 'Yulia', 'Zahra'];
  const lastNames = ['Pratama', 'Hidayat', 'Saputra', 'Wijaya', 'Nugroho', 'Kusuma', 'Santoso', 'Ramadhan', 'Setiawan', 'Utama', 'Suryana', 'Hadi', 'Prakoso', 'Fadhilah', 'Firmansyah'];

  const emptySulung = {
    sehat: 0, karies: 0, dicabutKaries: 0, tumpatanKaries: 0,
    tumpatanTanpaKaries: 0, dicabutSebabLain: 0, fissureSealant: 0,
    protesaCekat: 0, tidakTumbuh: 0, lainLain: 0
  };

  for (let i = 0; i < 150; i++) {
    const isMale = i < 75; // Exactly 75 Male (JK_CODE 1) & 75 Female (JK_CODE 2)
    const gender = isMale ? 'Laki-laki' : 'Perempuan';
    const firstName = isMale ? maleNames[i % 25] : femaleNames[(i - 75) % 25];
    const lastName = lastNames[i % 15];
    const nama = `${firstName} ${lastName} (${i + 1})`;
    const umur = 18 + (i % 45); // 18-62 years old

    // Bervariasi secara statistik dengan simpangan baku (SD) > 0
    const karies = isMale ? (1 + (i % 6)) : (2 + (i % 7));
    const dicabut = isMale ? (i % 4) : (1 + (i % 4));
    const tumpatan = isMale ? (i % 3) : (i % 4);
    const dmft = karies + dicabut + tumpatan;

    const gTetap = {
      sehat: Math.max(0, 28 - dmft),
      karies,
      dicabutKaries: dicabut,
      tumpatanKaries: 0,
      tumpatanTanpaKaries: tumpatan,
      dicabutSebabLain: 0,
      fissureSealant: 0,
      protesaCekat: 0,
      tidakTumbuh: 0,
    ...
    const gusiBerdarah = (i % 2 === 0);
    const lesiMukosa = (i % 5 === 0);
    const perluPerawatanSegera = dmft > 3;
    const perluDirujuk = perluPerawatanSegera || (i % 3 === 0);

    const resp: RespondentData = {
      nama,
      nik: `3201${isMale ? '01' : '41'}${String(10 + (i % 80)).padStart(2, '0')}01950${String(i + 1).padStart(3, '0')}`,
      jenisKelamin: gender,
      umur,
      kelompokUmur: '18-59',
      pendidikan: i % 2 === 0 ? 'SMA' : 'S1/D4',
      pekerjaan: i % 2 === 0 ? 'PEGAWAI SWASTA' : 'WIRASWASTA/WIRAUSAHA',
      tanggalInput: today,
      pemeriksa: 'Arini Haerunnisa',
      ...
      tindakLanjut: {
        ...
        perluPerawatanTidakSegera: !perluPerawatanSegera,
        perluDirujuk,
        dirujukKe: perluDirujuk ? (i % 2 === 0 ? 'puskesmas' : 'rsgm_rskgm') : 'tidak_dirujuk'
      },
...
export const generate50BalancedTTestRespondents = generate150BalancedTTestRespondents;
